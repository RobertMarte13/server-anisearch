...
router.get("/api/comment", async (req, res) => {
  try {
    ...
    const [rows] = await pool.query("SELECT username, comment, auth_id, commentIdSubComment, fechaCreacion FROM auth INNER JOIN comments ON auth_id = comment_id limit 15");
