...
router.post("/api/subcomment", async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      "INSERT INTO subcomts(comments, likes, sub_comment_id, auth_comment_id, commentIdSubComment2, fechaCreacion, sub_delete_id) VALUES (?, ?, ?, ?, ?, ?, ?)",
      [comments, likes, sub_comment_id, auth_comment_id, commentIdSubComment2, fechaCreacion, sub_delete_id]
    );
