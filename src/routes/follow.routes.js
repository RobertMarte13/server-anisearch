...
router.post('/api/follower', async (req, res) => {
    ...
    try {
        ...
        await pool.query("INSERT INTO follow(users_id, user_id, delete_id) VALUE (?, ?, ?)",[users_id, user_id, delete_id])
...
router.get('/api/follower', async (req, res) => {
    ...
    try {
        ...
        const [rows] = await pool.query("SELECT users_id, COUNT(users_id) AS follow_count FROM follow GROUP BY users_id");
...
router.delete('/api/follower/:delete_id', async (req, res) => {
    ...
    try {
        ...
        await pool.query("DELETE FROM follow WHERE delete_id = ?", [req.params.delete_id]);
